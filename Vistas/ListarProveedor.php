...
// Total de filas
$totalStmt = $db->prepare("SELECT COUNT(*) FROM proveedores{$where}");
$totalStmt->execute($params);
$totalRows  = (int)$totalStmt->fetchColumn();
$totalPages = (int)ceil($totalRows / $limit);

// Consulta paginada
$sql = "SELECT * FROM proveedores{$where} ORDER BY nombre ASC LIMIT :limit OFFSET :offset";
$stmt = $db->prepare($sql);
...
<head>
  ...
  <title>Lista de Proveedores - NextGen</title>
  ...
  <nav class="sidebar">
    ...
    <div class="menu-item open">
      ...
      <div class="submenu">
        ...
        <a href="ListarProveedor.php" class="active" >Proveedores</a>
      </div>
    </div>
    ...
    <main class="main-content">
      ...
      <section class="content">
        ...
        <div class="actions">
          <form method="GET">
            <input type="text" name="q" placeholder="Buscar proveedor..." value="<?= htmlspecialchars($search) ?>">
            <button type="submit" class="btn-primary"><i class="bi bi-search"></i></button>
          </form>
          <a href="RegistrarProveedor.php" class="btn-primary"><i class="bi bi-plus-lg"></i> Nuevo Proveedor</a>
        </div>
          ...
          <nav class="pagination">
            ...
            <?php for ($p = 1; $p <= $totalPages; $p++): ?>
              <a href="?q=<?= urlencode($search) ?>&page=<?= $p ?>"
                 class="btn-primary <?= $p === $page ? 'active' : '' ?>">
                <?= $p ?>
              </a>
            <?php endfor; ?>
