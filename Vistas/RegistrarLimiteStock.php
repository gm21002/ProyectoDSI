...
<head>
  ...
  <title>Configurar Límite de Stock - NextGen</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Ajuste para centrar el formulario vertical y horizontalmente */
    .form-wrapper {
      flex: 1;
      display: flex;
    ...
    /* Inputs y selects iguales */
    .form-container input,
    .form-container select.select-custom {
      width: 100%;
      padding: 10px;
      margin-bottom: 20px;
      border-radius: 8px;
    ...
    .form-container select.select-custom:hover {
      background-color: #f1f5f9;
    }

    .form-container input:focus,
    .form-container select.select-custom:focus {
      outline: none;
      box-shadow: 0 0 0 2px #2563eb;
      background-color: #f9fafb;
    }

    .form-container select.select-custom option {
      color: #111827;
    }
...
<body>
      ...
      <section class="content">
        <div class="form-wrapper">
          <div class="form-container">
            <h1>Configurar Límite de Stock</h1>

            <?php if (!empty($_SESSION['errores'])): ?>
              <div class="error">
                <?= implode('<br>', $_SESSION['errores']); unset($_SESSION['errores']); ?>
              </div>
            ...
            <form action="../Controladores/LimiteStockController.php" method="post">
              <label for="producto_id">Producto</label>
              <select id="producto_id" name="producto_id" class="select-custom" required>
              ...
              <label for="stock_minimo">Límite Mínimo</label>
              <input id="stock_minimo" type="number" name="stock_minimo" min="0" required>

              <label for="stock_maximo">Límite Máximo</label>
              <input id="stock_maximo" type="number" name="stock_maximo" min="0" required>
  ...
  <script>
    ...
    // Cargar límites existentes al seleccionar producto
    const prodSel = document.getElementById('producto_id');
    const minIn   = document.getElementById('stock_minimo');
    const maxIn   = document.getElementById('stock_maximo');
    prodSel.addEventListener('change', () => {
      const id = prodSel.value;
      minIn.value = '';
      maxIn.value = '';
      if (!id) return;
      fetch(`../Controladores/LimiteStockController.php?accion=obtener&producto_id=${id}`)
        .then(res => res.json())
        .then(data => {
          minIn.value = data.stock_minimo;
          maxIn.value = data.stock_maximo;
        })
        .catch(err => console.error('Error al cargar límites:', err));
    });
  </script>
</body>
</html>
