...
<body>
  ...
  <h2>Comparar Stock Físico vs Inventario del Sistema</h2>

  <div class="actions">
    <form action="../Controladores/CompararInventarioController.php" method="POST" enctype="multipart/form-data">
      <input type="file" name="archivo_csv" accept=".csv" required />
      <button type="submit" class="btn-primary">
        <i class="bi bi-arrow-repeat"></i> Comparar Inventario
      </button>
    </form>
  </div>

  <?php if (isset($_SESSION['reporte_comparacion'])): ?>
    <form action="../Controladores/ExportarDiferenciasController.php" method="POST">
      <div class="table-container">
        <table>
          ...
          <tbody>
            <?php foreach ($_SESSION['reporte_comparacion'] as $index => $r): ?>
              <tr>
                <td>
                  <?php if ($r['estado'] === 'Diferente'): ?>
                    <input type="checkbox" name="marcados[]" value="<?= $index ?>">
                  <?php endif; ?>
                </td>
                ...
                <td><?= htmlspecialchars($r['stock_sistema']) ?></td>
                <td><?= htmlspecialchars($r['stock_fisico']) ?></td>
                <td><?= htmlspecialchars($r['diferencia']) ?></td>
                <td><span class="badge bg-<?= $r['estado'] === 'Coincide' ? 'success' : ($r['estado'] === 'No encontrado' ? 'warning' : 'danger') ?>">
                  <?= $r['estado'] ?>
                </span></td>
              </tr>
            <?php endforeach; ?>
      ...
      <button type="submit" class="btn-primary" style="margin-top: 16px;">
        <i class="bi bi-file-earmark-arrow-down"></i> Exportar Seleccionados
      </button>
    </form>
    <?php // unset($_SESSION['reporte_comparacion']); ?>
  <?php endif; ?>
</section>
